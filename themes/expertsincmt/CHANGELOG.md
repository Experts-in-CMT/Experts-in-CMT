...
- **Entry-Point Cards: "Where to start?" Shortcode with Per-Page Sets (`entry-points-shortcode.php`, `entry-points.css`)**
  - The homepage and the Genetics context page each carried a hand-built Stackable column set of clickable panels, one per destination, styled apart from the rest of the site and re-edited in the block editor on every page that used them. A new `[eic_entry_points]` shortcode replaces those with hardcoded, single-file card sets that render in the genes/subtype card system: the shared card tokens (radius, shadow, speed, ease), a light-weight navy title, bare navy line-icons in the app heroes' stroke style, the house scale-on-hover, and a two-by-two grid that caps its own width so the cards keep a comfortable measure instead of stretching wide and sparse.
  - Each card is a clickable panel built on the accessible clickable-card pattern: the title carries the one real link, and a stretched `::after` overlays the whole panel, so the entire card is a click target while a screen reader announces a single link per card. A bottom "Explore" cue is pinned to the card foot (decorative, `aria-hidden`), its arrow sliding on hover, filling the card's lower whitespace and giving first-time visitors an explicit "this is clickable" signal.
  - Named sets let different pages show different cards from one file: `set="home"` (the default) renders the four "Where to start?" on-ramps (What Is CMT?, The Database, Variant Mechanisms, Genetic Testing), and `set="genetics"` renders the four Genetics-nav destinations (CMT Classifications, CMT Genetics Database, Variant Mechanisms, Genetic Testing). A set carries a default lead label (the home set uses "Start here if:"), and any card may override it with its own tagline (the genetics set gives each card one: "What's in a name?", "CMT. Curated.", "The how, not the what", "The info that matters"); a lead that resolves to empty renders no label. Adding a third page later is one more named set. An optional `heading` attribute renders an `<h2>` above the grid, for a surface with no section heading of its own, such as a 404 template used as a reentry point.
  - Icons follow the heroes: an info mark, an open book (reused from the Genes DB hero) for Classifications, a search glass for the database, the Variant Mechanisms categories branch, and the Genes DB DNA double-helix for Genetic Testing. The helix flattens into an "S" at the 28px card size, so it renders a touch larger (34px, via an `--lg` icon modifier) on the Genetic Testing card, where it reads as a double helix.
