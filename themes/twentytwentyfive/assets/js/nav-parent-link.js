...
(function () {
  ...
  function forceNavigate(link, event) {
    ...
    if (event) {
      ...
      try { if (event.stopImmediatePropagation) event.stopImmediatePropagation(); } catch {}
    }
    log('navigate ->', href);
  ...
  function setupItem(item) {
    ...
    link.addEventListener('pointerup', navHandler, { capture: true, passive: false });
    link.addEventListener('click', navHandler, { capture: true, passive: false });
    link.addEventListener('touchend', navHandler, { capture: true, passive: false });
...
const NAV_ROUTES = [
  { text: 'CMT Genetics', url: '/cmt-genetics' },
];
...
function bindHardRoute(el, url) {
  ...
  ['pointerup', 'click', 'touchend'].forEach((evt) =>
    el.addEventListener(evt, navNow, { capture: true, passive: false })
  );
  el.addEventListener(
    'keydown',
    (e) => {
