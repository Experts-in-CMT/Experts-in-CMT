...
(function () {
  ...
  function findTarget(items, current, dir) {
    ...
    const dist = {
      right: el => ({ primary: Math.max(0, center(el).x - c0.x), cross: Math.abs(center(el).y - c0.y) }),
      left:  el => ({ primary: Math.max(0, c0.x - center(el).x), cross: Math.abs(center(el).y - c0.y) }),
      down:  el => ({ primary: Math.max(0, center(el).y - c0.y), cross: Math.abs(center(el).x - c0.x) }),
      up:    el => ({ primary: Math.max(0, c0.y - center(el).y), cross: Math.abs(center(el).x - c0.x) }),
    }[dir];
