...
function eic_ajax_dr_get_posts() {
  ...
  // Params
  $qs      = isset($_POST['qs']) ? sanitize_text_field(wp_unslash($_POST['qs'])) : '';
  $dr_cat  = isset($_POST['dr_cat']) ? sanitize_text_field(wp_unslash($_POST['dr_cat'])) : '';
  $dr_sort = isset($_POST['dr_sort']) ? sanitize_text_field(wp_unslash($_POST['dr_sort'])) : '';
  ...
  // Sort
  switch ($dr_sort) {
    case 'title_az': $args['orderby'] = 'title'; $args['order'] = 'ASC'; break;
    case 'title_za': $args['orderby'] = 'title'; $args['order'] = 'DESC'; break;
  ...
  if (!empty($rows)) {
    foreach ($rows as $i => $row_items) {
      ...
      echo '<div class="dr-row' . ($is_last ? ' dr-row--last' : '') . '"' . ($is_last ? ' data-count="'.(int)$count.'"' : '') . '>';
  ...
  $big = 999999;
  echo paginate_links([
    'base'      => str_replace($big, '%#%', esc_url(add_query_arg('dr_paged', $big))),
    'format'    => '&dr_paged=%#%',
    'current'   => max(1, $paged),
