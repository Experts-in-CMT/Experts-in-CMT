...
add_action('pre_get_terms', function ( $query ) {
    ...
    $is_target = is_array($tax) ? (bool) array_intersect( $tax, $targets ) : in_array( $tax, $targets, true );
    if ( ! $is_target ) return;
...
add_filter('wp_terms_checklist_args', function ( $args, $post_id ) {
    if ( ! empty( $args['taxonomy'] ) && in_array( $args['taxonomy'], eicmt_ordered_taxonomies(), true ) ) {
