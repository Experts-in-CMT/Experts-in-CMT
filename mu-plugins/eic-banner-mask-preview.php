 ...
 *     driven by banner_fade_start / banner_fade_end.
 *   • Mobile: a stacked preview tile below the thumbnail, driven by
 *     banner_fade_start_mobile / banner_fade_end_mobile.
 *
 * Both update live as their sliders move, so the fade can be dialed
 * in per breakpoint without a save-and-check loop.
 *
 * Fields (header-banner-fields.php):
 *   banner_image              (image)          field_6792eabc684aa
 *   banner_fade_start          (range 0..100)  field_eic_banner_fade_start
 *   banner_fade_end            (range 0..100)  field_eic_banner_fade_end
 *   banner_fade_start_mobile   (range 0..100)  field_eic_banner_fade_start_mobile
 *   banner_fade_end_mobile     (range 0..100)  field_eic_banner_fade_end_mobile
 *
 * Location: wp-content/mu-plugins/eic-banner-mask-preview.php
 */
...
add_action("admin_enqueue_scripts", function ($hook) {
    ...
    // --- Inline styles ---
    wp_register_style("eic-banner-mask-preview", false);
    wp_enqueue_style("eic-banner-mask-preview");
    wp_add_inline_style(
        "eic-banner-mask-preview",
        <<<CSS
.acf-field[data-name="banner_image"] .image-wrap {
    position: relative;
    overflow: hidden;
    border-radius: 6px;
}
...
.eic-mask-preview-mobile__img {
    display: block;
    width: 100%;
    height: auto;
}
.eic-mask-preview-mobile__mask {
    position: absolute;
    inset: 0;
    pointer-events: none;
    z-index: 1;
}
.eic-mask-preview__edge {
    position: absolute;
    top: 0;
    bottom: 0;
    width: 0;
...
CSS
    ...
    // --- Inline script ---
    wp_register_script("eic-banner-mask-preview", "", [], null, true);
    wp_enqueue_script("eic-banner-mask-preview");
...
(function () {
    ...
    var START = "banner_fade_start";
    var END = "banner_fade_end";
    var START_M = "banner_fade_start_mobile";
    var END_M = "banner_fade_end_mobile";
    ...
    function placeMarkers(root, s, e) {
        var es = root.querySelector(".eic-mask-preview__edge--start");
        var ee = root.querySelector(".eic-mask-preview__edge--end");
        if (es) es.style.left = s + "%";
        if (ee) ee.style.left = e + "%";
        var ts = root.querySelector(".eic-mask-preview__tag--start");
        var te = root.querySelector(".eic-mask-preview__tag--end");
        if (ts) { ts.style.left = s + "%"; ts.textContent = s + "%"; }
        if (te) { te.style.left = e + "%"; te.textContent = e + "%"; }
    }

    var MARKERS =
        '<span class="eic-mask-preview__edge eic-mask-preview__edge--start"></span>' +
        '<span class="eic-mask-preview__edge eic-mask-preview__edge--end"></span>' +
        '<span class="eic-mask-preview__tag eic-mask-preview__tag--start"></span>' +
        '<span class="eic-mask-preview__tag eic-mask-preview__tag--end"></span>';
    ...
    function updateDesktop(hasImage) {
        ...
        if (!wrap) return;
        if (!hasImage) {
            var stale = wrap.querySelector(".eic-mask-preview");
            if (stale) stale.remove();
            var staleLabel = wrap.querySelector(".eic-mask-preview__label");
        ...
        var ov = wrap.querySelector(".eic-mask-preview");
        if (!ov) {
            ov = document.createElement("div");
            ov.className = "eic-mask-preview";
            ov.innerHTML = MARKERS;
            wrap.appendChild(ov);
        }
        if (!wrap.querySelector(".eic-mask-preview__label")) {
            var lab = document.createElement("div");
            lab.className = "eic-mask-preview__label";
    ...
    function updateMobile(src) {
        ...
        var tile = parent.querySelector(".eic-mask-preview-mobile");
        ...
        if (!tile) {
            ...
            tile.className = "eic-mask-preview-mobile";
            tile.innerHTML =
                '<img class="eic-mask-preview-mobile__img" alt="" />' +
                '<div class="eic-mask-preview-mobile__mask"></div>' +
                MARKERS +
                '<div class="eic-mask-preview__label">Mobile</div>';
        ...
        var im = tile.querySelector(".eic-mask-preview-mobile__img");
        if (im && im.getAttribute("src") !== src) im.setAttribute("src", src);
        var mask = tile.querySelector(".eic-mask-preview-mobile__mask");
    ...
    function init() {
        var f = field(IMG);
        if (!f) return; // banner group not on this screen
