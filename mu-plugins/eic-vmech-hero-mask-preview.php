 ...
 *     var(--bg) 0%,
 *     var(--bg) var(--vmech-hero-fade-start),
 ...
 *     driven by vmech_hero_fade_start / vmech_hero_fade_end.
 *   • Mobile: a stacked preview tile below the thumbnail, driven by
 *     vmech_hero_fade_start_mobile / vmech_hero_fade_end_mobile.
 *
 ...
 *
 * Fields (variant-mechanism-hero-fields.php):
 *   vmech_hero_image               (image)         field_eic_vmech_hero_image
 *   vmech_hero_fade_start          (range 0..100)  field_eic_vmech_hero_fade_start
 *   vmech_hero_fade_end            (range 0..100)  field_eic_vmech_hero_fade_end
 *   vmech_hero_fade_start_mobile   (range 0..100)  field_eic_vmech_hero_fade_start_mobile
 *   vmech_hero_fade_end_mobile     (range 0..100)  field_eic_vmech_hero_fade_end_mobile
 *
 * Location: wp-content/mu-plugins/eic-vmech-hero-mask-preview.php
 */
...
add_action("admin_enqueue_scripts", function ($hook) {
    ...
    // --- Inline styles ---
    wp_register_style("eic-vmech-hero-mask-preview", false);
    wp_enqueue_style("eic-vmech-hero-mask-preview");
    wp_add_inline_style(
        "eic-vmech-hero-mask-preview",
        <<<CSS
.acf-field[data-name="vmech_hero_image"] .image-wrap {
    position: relative;
    overflow: hidden;
    border-radius: 6px;
}
...
.eic-vmh-preview-mobile__img {
    display: block;
    width: 100%;
    height: auto;
}
.eic-vmh-preview-mobile__mask {
    position: absolute;
    inset: 0;
    pointer-events: none;
    z-index: 1;
}
.eic-vmh-preview__edge {
    position: absolute;
    top: 0;
    bottom: 0;
    width: 0;
...
CSS
    ...
    // --- Inline script ---
    wp_register_script("eic-vmech-hero-mask-preview", "", [], null, true);
    wp_enqueue_script("eic-vmech-hero-mask-preview");
...
(function () {
    ...
    var IMG = "vmech_hero_image";
    var START = "vmech_hero_fade_start";
    var END = "vmech_hero_fade_end";
    var START_M = "vmech_hero_fade_start_mobile";
    var END_M = "vmech_hero_fade_end_mobile";
    ...
    function placeMarkers(root, s, e) {
        var es = root.querySelector(".eic-vmh-preview__edge--start");
        var ee = root.querySelector(".eic-vmh-preview__edge--end");
        if (es) es.style.left = s + "%";
        if (ee) ee.style.left = e + "%";
        var ts = root.querySelector(".eic-vmh-preview__tag--start");
        var te = root.querySelector(".eic-vmh-preview__tag--end");
        if (ts) { ts.style.left = s + "%"; ts.textContent = s + "%"; }
        if (te) { te.style.left = e + "%"; te.textContent = e + "%"; }
    }

    var MARKERS =
        '<span class="eic-vmh-preview__edge eic-vmh-preview__edge--start"></span>' +
        '<span class="eic-vmh-preview__edge eic-vmh-preview__edge--end"></span>' +
        '<span class="eic-vmh-preview__tag eic-vmh-preview__tag--start"></span>' +
        '<span class="eic-vmh-preview__tag eic-vmh-preview__tag--end"></span>';
    ...
    function updateDesktop(hasImage) {
        ...
        if (!wrap) return;
        if (!hasImage) {
            var stale = wrap.querySelector(".eic-vmh-preview");
            if (stale) stale.remove();
            var staleLabel = wrap.querySelector(".eic-vmh-preview__label");
        ...
        var ov = wrap.querySelector(".eic-vmh-preview");
        if (!ov) {
            ov = document.createElement("div");
            ov.className = "eic-vmh-preview";
            ov.innerHTML = MARKERS;
            wrap.appendChild(ov);
        }
        if (!wrap.querySelector(".eic-vmh-preview__label")) {
            var lab = document.createElement("div");
            lab.className = "eic-vmh-preview__label";
    ...
    function updateMobile(src) {
        ...
        var tile = parent.querySelector(".eic-vmh-preview-mobile");
        ...
        if (!tile) {
            ...
            tile.className = "eic-vmh-preview-mobile";
            tile.innerHTML =
                '<img class="eic-vmh-preview-mobile__img" alt="" />' +
                '<div class="eic-vmh-preview-mobile__mask"></div>' +
                MARKERS +
                '<div class="eic-vmh-preview__label">Mobile</div>';
        ...
        var im = tile.querySelector(".eic-vmh-preview-mobile__img");
        if (im && im.getAttribute("src") !== src) im.setAttribute("src", src);
        var mask = tile.querySelector(".eic-vmh-preview-mobile__mask");
