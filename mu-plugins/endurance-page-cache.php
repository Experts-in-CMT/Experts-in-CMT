...
if (!class_exists("Endurance_Page_Cache")) {
    ...
    class Endurance_Page_Cache
    {
         ...
         * @var array
         */
        public $cache_exempt = ["checkout", "cart", "wp-admin"];
        ...
        public function __construct()
        {
            if (isset($_GET["doing_wp_cron"])) {
                ...
                add_action($this->epc_scheduled_purge_all_hook, [
                    __CLASS__,
                    "scheduled_purge_all",
                ]);
                return;
            ...
            $this->throttled = array_filter(
                (array) get_transient("epc_throttled")
            );

            $this->cache_level = $this->get_cache_level();
            $this->cache_dir = WP_CONTENT_DIR . "/endurance-page-cache";

            $cloudflare_state = get_option(
                "endurance_cloudflare_enabled",
                false
            );

            $this->cloudflare_enabled = (bool) $cloudflare_state;
            $this->cloudflare_tier =
                is_numeric($cloudflare_state) && $cloudflare_state
                    ? "basic"
                    : $cloudflare_state;
            $this->udev_api_services["cf"] = $this->cloudflare_tier;

            $path = defined("ABSPATH") ? ABSPATH : __DIR__;
            $this->file_based_enabled = (bool) get_option(
                "endurance_file_enabled",
                false === strpos($path, "public_html")
            );
        ...
        public function hooks()
        {
            if ($this->is_enabled("page")) {
                ...
                add_action("shutdown", [$this, "finish"]);
                add_action("shutdown", [$this, "shutdown"]);
                add_action("generate_rewrite_rules", [$this, "config_nginx"]);
            }
            add_filter(
                "mod_rewrite_rules",
                [$this, "htaccess_contents_rewrites"],
                77
            );
            add_filter(
                "mod_rewrite_rules",
                [$this, "htaccess_contents_expirations"],
                88
            );

            add_action("update_option_endurance_cache_level", [
                $this,
                "update_htaccess",
            ]);
            add_action("update_option_endurance_file_enabled", [
                $this,
                "update_htaccess",
            ]);
            add_action("update_option_epc_skip_404_handling", [
                $this,
                "update_htaccess",
            ]);
            add_action("update_option_epc_filetype_expirations", [
                $this,
                "update_htaccess",
            ]);
            add_action("delete_option_epc_filetype_expirations", [
                $this,
                "update_htaccess",
            ]);

            add_action("admin_init", [$this, "register_cache_settings"]);
            add_action("transition_post_status", [$this, "save_post"], 10, 3);
            add_action("edit_terms", [$this, "edit_terms"]);

            add_action("comment_post", [$this, "comment"]);

            add_action("updated_option", [$this, "option_handler"], 10, 3);

            add_action("epc_purge", [$this, "purge_all"]);
            add_action("epc_purge_request", [$this, "purge_request"]);

            add_action("wp_update_nav_menu", [$this, "purge_all"]);

            add_action("admin_bar_menu", [$this, "admin_toolbar"], 99);

            add_action("init", [$this, "do_purge"]);
            ...
            add_filter(
                "pre_update_option_mm_cache_settings",
                [$this, "cache_type_change"],
                10,
                2
            );
            add_filter(
                "pre_update_option_endurance_cache_level",
                [$this, "cache_level_change"],
                10,
                2
            );

            add_filter("got_rewrite", [$this, "force_rewrite"]);

            add_action("shutdown", [$this, "udev_cache_purge_via_buffer"]);
        }
        ...
        public function admin_toolbar($wp_admin_bar)
        {
            if (current_user_can("manage_options") && $this->is_enabled()) {
                ...
                $args = [
                    ...
                    "href" => add_query_arg(["epc_purge_all" => true]),
                ];
                $wp_admin_bar->add_node($args);
                ...
                if (!is_admin()) {
                    $args = [
                        ...
                        "parent" => "epc_purge_menu",
                        "href" => add_query_arg(["epc_purge_single" => true]),
                    ];
                    $wp_admin_bar->add_node($args);
                }
        ...
        public function register_cache_settings()
        {
            ...
            add_settings_field(
                ...
                "Cache Level",
                [$this, "output_cache_settings"],
                "general",
                $section_name,
                ["field" => "endurance_cache_level"]
            );
            add_settings_field(
                "epc_skip_404_handling",
                "Skip WordPress 404 Handling For Static Files",
                function () {
                    echo '<input type="checkbox" name="epc_skip_404_handling" value="1"' .
                        checked(
                            (bool) get_option("epc_skip_404_handling"),
                            true,
                            false
                        ) .
                        " />";
        ...
        public function to_snake_case($value, $delimiter = "_")
        {
            if (!ctype_lower($value)) {
                ...
                $value = trim(
                    mb_strtolower(
                        preg_replace(
                            '/([A-Z][A-Z0-9]*(?=$|[A-Z][a-z0-9])|[A-Za-z][a-z0-9]+)/u',
                            '$1' . $delimiter,
                            $value
                        ),
                        "UTF-8"
        ...
        public function option_handler($option, $old_value, $new_value)
        {
            ...
            $force_if_contains = ["html", "css", "style", "query", "queries"];
            ...
            if (ctype_upper(str_replace(["-", "_"], "", $option))) {
                $option = strtolower($option);
            }
            $option_name =
                "_" .
                $this->to_snake_case($this->to_studly_case($option)) .
                "_";
        ...
        public function schedule_purge_all()
        {
            if (!wp_next_scheduled($this->epc_scheduled_purge_all_hook)) {
                wp_schedule_single_event(
                    time() + 60,
                    $this->epc_scheduled_purge_all_hook
                );
            }
        }
        ...
        public function write($page)
        {
            ...
            if (!empty($page)) {
                ...
                if (
                    file_exists($path . "_index.html") &&
                    filemtime($path . "_index.html") > time() - HOUR_IN_SECONDS
                ) {
                    return $page;
                }
        ...
        public function purge_cdn()
        {
            if (
                !$this->force_purge &&
                true === $this->should_throttle("sitelock_cdn", __METHOD__)
            ) {
                return;
            }
            ...
            if ("BlueHost" === get_option("mm_brand")) {
                ...
                $query = add_query_arg(["domain" => $domain], $endpoint);
        ...
        public function purge_cdn_single($pattern = "")
        {
            if (
                !$this->force_purge &&
                true === $this->should_throttle($pattern, __METHOD__)
            ) {
                return;
            }
        ...
        public function should_throttle($uri, $type)
        {
            ...
            if (
                isset($this->throttled[$type], $this->throttled[$type][$hash])
            ) {
        ...
        public function shutdown()
        {
            if ($this->should_update_throttled_items) {
                ...
                foreach ($this->throttled as $type => $group) {
                    $throttled[$type] = array_filter($group, [
                        $this,
                        "is_timestamp_valid",
                    ]);
                }
        ...
        public function purge_request($uri)
        {
            ...
            if (
                !$this->force_purge &&
                true === $this->should_throttle($uri, __METHOD__)
            ) {
                return;
            }
            ...
            wp_remote_request(
                $this->get_purge_request_url($uri, "http"),
                $args
            );
            wp_remote_request(
                $this->get_purge_request_url($uri, "https"),
                $args
            );
        ...
        public function purge_dir($dir = null)
        {
            if (
                !$this->force_purge &&
                true === $this->should_throttle($dir, __METHOD__)
            ) {
                return;
            }
        ...
        public function purge_all()
        {
            if (
                !$this->force_purge &&
                true === $this->should_throttle("all", __METHOD__)
            ) {
                return;
            }
        ...
        public function is_cachable($type = "default")
        {
            ...
            if ("file" === $type) {
                ...
                $cache_exempt = array_merge($this->cache_exempt, [
                    "@",
                    "%",
                    ":",
                    ";",
        ...
        {
            if (
                $this->is_cachable("file") &&
                $this->file_based_enabled &&
                ob_get_contents()
            ) {
                ob_end_clean();
            }
        ...
        public function htaccess_contents_rewrites($rules)
        {
            ...
            $cache_url =
                ...
                str_replace(
                    get_option("home"),
                    "",
                    WP_CONTENT_URL . "/endurance-page-cache"
                );
...
HTACCESS;
        ...
        public function htaccess_contents_expirations($rules)
        {
            ...
            $file_types = wp_parse_args(
                get_option("epc_filetype_expirations", []),
                $default_files
            );
        ...
        public function is_enabled($type = "page")
        {
            ...
            $incompatible_themes = ["headway", "prophoto"];
            ...
            if ("page" === $type) {
                if (isset($_GET["epc_toggle"]) && is_admin()) {
                    ...
                    if (in_array($_GET["epc_toggle"], $valid_values, true)) {
                        // phpcs:ignore WordPress.Security.NonceVerification
                ...
                if (
                    isset($cache_settings["page"]) &&
                    "disabled" === $cache_settings["page"]
                ) {
                    return false;
                } else {
                    return true;
            ...
            if ("browser" === $type) {
                ...
                if (
                    isset($cache_settings["browser"]) &&
                    "disabled" === $cache_settings["browser"]
                ) {
                    return false;
                } else {
                    return true;
        ...
        public function status_link($links)
        {
            if ($this->is_enabled()) {
                $links[] =
                    '<a href="' .
                    add_query_arg(["epc_toggle" => "disabled"]) .
                    '">Disable</a>';
            } else {
                $links[] =
                    '<a href="' .
                    add_query_arg(["epc_toggle" => "enabled"]) .
                    '">Enable</a>';
            }
            $links[] =
                '<a href="' .
                add_query_arg(["epc_purge_all" => "true"]) .
                '">Purge Cache</a>';
        ...
        {
            if (
                (isset($_GET["epc_purge_all"]) ||
                    isset($_GET["epc_purge_single"])) &&
                is_user_logged_in() &&
                ...
                header(
                    "Location: " .
                        remove_query_arg(["epc_purge_single", "epc_purge_all"])
                );
            }
        }
         ...
         * @return array
         */
        public function cache_type_change(
            $new_cache_settings,
            $old_cache_settings
        ) {
            $new_page_cache_value = 0;
            if (
                is_array($new_cache_settings) &&
                isset($new_cache_settings["page"])
            ) {
                $new_page_cache_value =
                    "enabled" === $new_cache_settings["page"] ? 1 : 0;
            }
        ...
        public function update_level_expirations($level)
        {
            ...
            $original_expirations = get_option("epc_filetype_expirations", []);
            switch ($level) {
                case 3:
                    $new_expirations = [
                        "image/jpg" => "1 week",
            ...
            $expirations = wp_parse_args(
                $new_expirations,
                $original_expirations
            );
        ...
        public function toggle_nginx($new_value = 0)
        {
            if (!$this->use_file_cache()) {
                ...
                @file_put_contents(
                    $path . ".cpanel/proxy_conf/" . $domain,
                    "cache_level=" . $new_value
        ...
        public function update($checked_data)
        {
            ...
            if (!$muplugins_details) {
                $muplugins_details = wp_remote_get(
                    "https://cdn.hiive.space/bluehost/mu-plugins.json"
                );
            ...
            if (
                is_wp_error($muplugins_details) ||
                !isset($muplugins_details["body"])
            ) {
                return $checked_data;
            }
                    ...
                    if (
                        ...
                        if (
                            ...
                            if (
                                !is_wp_error($file) &&
                                isset($file["body"]) &&
                                strpos($file["body"], $info["constant"]) &&
                                is_writable(
                                    WP_CONTENT_DIR . $info["destination"]
                                )
                            ) {
                                file_put_contents(
                                    WP_CONTENT_DIR . $info["destination"],
         ...
         * @return void
         */
        protected function udev_cache_purge(
            $resources = [],
            $override_services = []
        ) {
            global $wp_version;
            ...
            if (
                !$this->force_purge &&
                true === $this->should_throttle($throttle_key, __METHOD__)
            ) {
                return;
            }

            $hosts = [wp_parse_url(home_url(), PHP_URL_HOST)];
            $services = !empty($override_services)
                ? $override_services
                : $this->udev_api_services;

            if ($services["cf"] && $this->cloudflare_enabled) {
                $services["cf"] = $this->cloudflare_tier;
            }

            wp_remote_post($this->udev_cache_api_uri($services), [
                "blocking" => false,
        ...
        {
            if (
                is_array($this->udev_purge_buffer) &&
                !empty($this->udev_purge_buffer)
            ) {
